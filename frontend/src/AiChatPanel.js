...
export default function AiChatPanel() {
  ...
  const handleSend = async () => {
    ...
    try {
      // 绕过前端代理层，直接请求后端服务对应的 8080 端口，从而彻底消除由于代理层或浏览器同源策略导致的流式缓冲
      const backendUrl = `${window.location.protocol}//${window.location.hostname}:8080/ai`;
